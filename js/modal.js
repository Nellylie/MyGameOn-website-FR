...
document.querySelector('.close2').addEventListener('click', () => { //fermer le formulaire apres la confirmation avec la croix
  formMerci.style.display = 'none';//gestion des affichages
  modalbg.style.display = 'none';
})
...
function validateInput(
  ...
  const trueOrFalse = [ //tableau pour rassembler chaque valeur a filtrer des fonctions autour des inputs recuperer en argument
    firstGestion(first),
    lastGestion(last),
    emailGestion(email),
  ...
  for (let validator of trueOrFalse) {//une boucle for pour chaque element du tableau trueOrfalse
    if (validator === false) { //compare le retour de leur fonction a false, si vrai le compteur s'incremente
      countFalse++;
    }
  }
  if ((countFalse === 0)) { //si le compteur est a zero, ca veut dire qu'il n y a pas de false, donc on recuperer et en travail avec les valeurs dans les inputs
    console.log(
      'Vous êtes : ' +
        first.value +
        ' - ' +
...
function firstGestion(first) {//on filtre chacune des valeurs recupérer on les compare et on execute l'action a réaliser selon la nature de l'erreur
  if (first.value === '') {//si la valeur est vide
    errorEvent(first, 'le champ est vide!');
    return false;
  } else if (!checkInput(first.value)) {//si la valeur comparer au regex de la fonction appelée retourne faux
    errorEvent(
      first,
...
function lastGestion(last) {
  if (last.value === '') {
    ...
    return false;
  } else if (!checkInput(last.value)) {//recupere la valeur de la fonction qui compare la valeur du nom au pattern regex
    errorEvent(
      last,
      "Les lettres de l'alphabet sont à eux-seuls autorisés et minimum 2 caractères",
    );
...
function emailGestion(email) {
  if (email.value === '') {
    ...
    return false;
  } else if (!checkEmail(email.value)) {//recupere la valeur de la fonction qui compare la valeur du nom au pattern regex
...
function birthdateGestion(birthdate) {
  if (birthdate.value === '') {
    ...
    return false;
  } else if (checkBirthday(birthdate.value) == false) {//recupere la valeur de la fonction qui compare la date de naissance avec celle du jour actuel pour avoir une date pausible
